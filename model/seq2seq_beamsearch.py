...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
                                ...
                                curr_states_c=None, curr_states_h=None, curr_attn_distrs=None):
  ...
  topk_states_c = gather(curr_states_c, "_topk_states_c")
  topk_states_h = gather(curr_states_h, "_topk_states_h")
  topk_attn_distrs = gather(curr_attn_distrs, "_topk_attn_distrs")
  return topk_seq, topk_gathered_scores, topk_flags, topk_states_c, topk_states_h, topk_attn_distrs
...
def beam_search(symbols_to_logits_fn,
  ...
  alive_states_c = _expand_to_beam_size(init_state_c, beam_size)
  finished_states_c = _expand_to_beam_size(init_state_c, beam_size)
  alive_states_h = _expand_to_beam_size(init_state_h, beam_size)
  finished_states_h = _expand_to_beam_size(init_state_h, beam_size)

  alive_attn_distrs = tf.zeros(
      (batch_size, beam_size, 1, model_config.max_complex_sentence), dtype=tf.float32)
  finished_attn_distrs = tf.zeros(
      (batch_size, beam_size, 1, model_config.max_complex_sentence), dtype=tf.float32)

  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
                    curr_scores, curr_finished,
                    curr_states_c, curr_states_h, curr_attn_distrs,
                    finished_states_c, finished_states_h, finished_attn_distrs):
    """Given sequences and scores, will gather the top k=beam size sequences.
    ...
    finished_attn_distrs = tf.concat(
        [finished_attn_distrs,
         tf.zeros([batch_size, beam_size, 1, model_config.max_complex_sentence], tf.float32)], axis=2)
    ...
    curr_attn_distrs = tf.concat([finished_attn_distrs, curr_attn_distrs], axis=1)
    curr_states_c = tf.concat([finished_states_c, curr_states_c], axis=1)
    curr_states_h = tf.concat([finished_states_h, curr_states_c], axis=1)

    return compute_topk_scores_and_seq(
        curr_finished_seq, curr_finished_scores, curr_finished_scores,
        curr_finished_flags, beam_size, batch_size, "grow_finished",
        curr_states_c=curr_states_c, curr_states_h=curr_states_h, curr_attn_distrs=curr_attn_distrs)
  ...
  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished,
    ...
    return compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs,
                                       ...
                                       "grow_alive",
                                       curr_states_c=curr_states_c, curr_states_h=curr_states_h,
                                       curr_attn_distrs=curr_attn_distrs)

  def grow_topk(i, alive_seq, alive_log_probs, alive_states_c, alive_states_h, alive_attn_distrs):
    ...
    flat_states_c = tf.reshape(alive_states_c, [batch_size * beam_size, -1])
    flat_states_h = tf.reshape(alive_states_h, [batch_size * beam_size, -1])

    # (batch_size * beam_size, decoded_length)
    flat_logits, state_c, state_h, attn_distrs = symbols_to_logits_fn(flat_ids, flat_states_c, flat_states_h)
    ...
    topk_states_c = tf.gather_nd(
        tf.reshape(state_c, [batch_size, beam_size, model_config.dimension]),
        topk_coordinates)

    topk_states_h = tf.gather_nd(
        tf.reshape(state_h, [batch_size, beam_size, model_config.dimension]),
        topk_coordinates)

    topk_attn_distrs = tf.gather_nd(alive_attn_distrs, topk_coordinates)
    cur_attn_distrs = tf.expand_dims(
        tf.gather_nd(
            tf.reshape(attn_distrs, [batch_size, beam_size, model_config.max_complex_sentence]),
            topk_coordinates), axis=2)
    topk_attn_distrs = tf.concat([topk_attn_distrs, cur_attn_distrs], axis=2)

    return topk_seq, topk_log_probs, topk_scores, topk_finished, topk_states_c, topk_states_h, topk_attn_distrs

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, alive_states_c, finished_states_c, alive_states_h, finished_states_h,
                 alive_attn_distrs, finished_attn_distrs):
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished, topk_states_c, topk_states_h, topk_attn_distrs = grow_topk(
        i, alive_seq, alive_log_probs, alive_states_c, alive_states_h, alive_attn_distrs)
    alive_seq, alive_log_probs, _, alive_states_c, alive_states_h, alive_attn_distrs = grow_alive(
        topk_seq, topk_scores, topk_log_probs, topk_finished, topk_states_c, topk_states_h, topk_attn_distrs)
    finished_seq, finished_scores, finished_flags, finished_states_c, finished_states_h, finished_attn_distrs = grow_finished(
        finished_seq, finished_scores, finished_flags, topk_seq, topk_scores,
        topk_finished, topk_states_c, topk_states_h, topk_attn_distrs, finished_states_c, finished_states_h, finished_attn_distrs)

    return (i + 1, alive_seq, alive_log_probs, finished_seq, finished_scores,
            finished_flags, alive_states_c, finished_states_c,
            alive_states_h, finished_states_h,
            alive_attn_distrs, finished_attn_distrs)

  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   finished_scores, finished_in_finished,
                   alive_states_c, finished_states_c, alive_states_h, finished_states_h,
                   alive_attn_distrs, finished_attn_distrs):
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, alive_states_c, finished_states_c, alive_states_h, finished_states_h,
   alive_attn_distrs,finished_attn_distrs) = tf.while_loop(
       _is_finished,
       inner_loop, [
       ...
       shape_invariants=[
           ...
           alive_states_c.get_shape(),
           finished_states_c.get_shape(),
           alive_states_h.get_shape(),
           finished_states_h.get_shape(),
  ...
  alive_attn_distrs.set_shape((None, beam_size, None, model_config.max_complex_sentence))
  finished_attn_distrs.set_shape((None, beam_size, None, model_config.max_complex_sentence))
  ...
  finished_attn_distrs = tf.where(
      tf.reduce_any(finished_flags, 1), finished_attn_distrs, alive_attn_distrs)
  return finished_seq, finished_scores, finished_attn_distrs
