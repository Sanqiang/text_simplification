...
public class Optimizer {
  ...
  public double[] run_Optimizer() {
    ...
    try {
      ...
      //System.out.println("DEBUG-Optimizer-finalLambda:");      
    ...
    for (int i = 0; i < sentNum; i++) {
      ...
      for (Iterator<String> it = candSet.iterator(); it.hasNext();) {
        ...
        if (maxModelScore < modelScore) {
          ...
          System.out.println("DEBUG-Optimizer-computeCorpusMetricScore: " + candStr);
        }
      }

      for (int j = 0; j < suffStatsCount; j++) {
        System.out.println("DEBUG-Optimizer-computeCorpusMetricScore: j=" + j + " " + tmpStatsVal[j]);
...
//      System.err.println("Score: " + evalMetric.score(statVal));
    ...
    System.out.println("Total execution time of Optimizer-compute_Score: " + cands.length + " cands.length " + (endTime - startTime));
