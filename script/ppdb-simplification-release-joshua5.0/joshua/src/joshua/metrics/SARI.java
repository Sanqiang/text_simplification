...
public class SARI extends EvaluationMetric
{
	...
	public int[] suffStats(String cand_str, int i) 
	{
		...
		for (int n = 1; n <= maxGramLength; ++n) {
			...
			stats[StatIndex.values().length * (n - 1) +  StatIndex.DELREF.ordinal()]  = src_sub_ref.keySet().size() * 8;
			...
			stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPBOTH.ordinal()] = (int) ( 1000000 * sumHashMapByDoubleValues(divideHashMap(src_and_cand_and_ref,src_and_cand))) ;
			stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPCAND.ordinal()] = (int) sumHashMapByDoubleValues(divideHashMap(src_and_cand_and_ref,src_and_ref));
			stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPREF.ordinal()]  = src_and_ref.keySet().size();
			
			//stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPBOTH2.ordinal()] = (int) sumHashMapByDoubleValues(divideHashMap(src_and_cand_and_ref,src_and_ref)) * 100000000 / src_and_ref.keySet().size() ;
			//stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPREF.ordinal()]  = src_and_ref.keySet().size() * 8;
	...
	public double score(int[] stats)
	{
		...
		for (int n = 1; n <= maxGramLength; ++n) {
			...
			//System.out.println("\nDEBUG-SARI:" + addCandCorrectNgram + " " + addCandTotalNgram + " " + addRefTotalNgram);
			...
			//int keepCandCorrectNgram2 = stats[StatIndex.values().length * (n - 1) +  StatIndex.KEEPBOTH2.ordinal()]; 
			...
			//System.out.println("\nDEBUG-SARI-KEEP: " + n + " " + keepCandCorrectNgram + " " + keepCandTotalNgram + " " + keepCandTotalNgram + " " + keepRefTotalNgram);
			...
			//System.out.println("\nDEBUG-SARI: " + n + " " + prec_add_n + " " + recall_add_n + " " + prec_del_n + " " + recall_del_n + " " + prec_keep_n + " " + recall_keep_n);
			
			//System.out.println("\nDEBUG-SARI-KEEP: " + n + " " + keepCandCorrectNgram + " " + keepCandTotalNgram + " " + keepRefTotalNgram);
			//System.out.println("\nDEBUG-STAR:" + n + " " + prec_del_n + " " + recall_del_n + " " + delCandCorrectNgram + " " + delRefTotalNgram);
		}
	...
	public HashMap<String, Double> divideHashMap(HashMap<String, Integer> counter1, HashMap<String, Integer> counter2) {
	...
	private enum StatIndex {KEEPBOTH, KEEPCAND, KEEPREF, DELBOTH, DELCAND, DELREF, ADDBOTH, ADDCAND, ADDREF, KEEPBOTH2};
