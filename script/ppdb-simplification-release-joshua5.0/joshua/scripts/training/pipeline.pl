...
# subsample
		if ($DO_SUBSAMPLE) {
			...
			$cachepipe->cmd("subsample",
											"java -Xmx20g -Dfile.encoding=utf8 -cp $JOSHUA/bin:$JOSHUA/lib/commons-cli-2.0-SNAPSHOT.jar joshua.subsample.Subsampler -e $TARGET -f $SOURCE -epath $DATA_DIRS{train}/ -fpath $DATA_DIRS{train}/ -output $DATA_DIRS{train}/subsampled/subsampled.$MAXLEN -ratio 1.04 -test $DATA_DIRS{train}/subsampled/test-data -training $DATA_DIRS{train}/subsampled/manifest",
											"$DATA_DIRS{train}/subsampled/manifest",
											"$DATA_DIRS{train}/subsampled/test-data",
											$TRAIN{source},
											$TRAIN{target},
...
if ($DO_FILTER_TM and ! $DOING_LATTICES and ! defined $TUNE_GRAMMAR_FILE) {
  ...
  $cachepipe->cmd("filter-tune",
									"$CAT $GRAMMAR_FILE | java -Xmx20g -Dfile.encoding=utf8 -cp $JOSHUA/class joshua.tools.TestSetFilter $FILTERING -v $TUNE{source} | $SCRIPTDIR/training/filter-rules.pl -bus$SCOPE | gzip -9n > $TUNE_GRAMMAR",
									$GRAMMAR_FILE,
									$TUNE{source},
									$TUNE_GRAMMAR);
}
...
if (! defined $GLUE_GRAMMAR_FILE) {
  $cachepipe->cmd("glue-tune",
                  "java -Xmx20g -cp $JOSHUA/lib/*:$THRAX/bin/thrax.jar edu.jhu.thrax.util.CreateGlueGrammar $TUNE_GRAMMAR > $DATA_DIRS{tune}/grammar.glue",
                  $TUNE_GRAMMAR_FILE,
...
for my $run (1..$OPTIMIZER_RUNS) {
  ...
  if ($TUNER eq "mert") {
		$cachepipe->cmd("mert-$run",
										"java -d64 -Xmx$TUNER_MEM -cp $JOSHUA/class joshua.zmert.ZMERT -maxMem 40000 $tunedir/mert.config > $tunedir/mert.log 2>&1",
										$TUNE_GRAMMAR_FILE,
		...
		$cachepipe->cmd("pro-$run",
										"java -d64 -Xmx$TUNER_MEM -cp $JOSHUA/class joshua.pro.PRO -maxMem 40000 $tunedir/pro.config > $tunedir/pro.log 2>&1",
										$TUNE_GRAMMAR_FILE,
...
# filter the test grammar
  ...
  if ($TEST_GRAMMAR_FILE) {
    ...
    if ($DO_FILTER_TM and ! $DOING_LATTICES) {
      ...
      $cachepipe->cmd("filter-test",
                      "$SCRIPTDIR/training/scat $GRAMMAR_FILE | java -Xmx20g -Dfile.encoding=utf8 -cp $JOSHUA/class joshua.tools.TestSetFilter $FILTERING -v $TEST{source} | $SCRIPTDIR/training/filter-rules.pl -bus$SCOPE | gzip -9n > $TEST_GRAMMAR",
                      $GRAMMAR_FILE,
                      $TEST{source},
                      $TEST_GRAMMAR);
    }
  ...
  if (! defined $GLUE_GRAMMAR_FILE) {
    ...
    "java -Xmx20g -cp $JOSHUA/lib/*:$THRAX/bin/thrax.jar edu.jhu.thrax.util.CreateGlueGrammar $TEST_GRAMMAR > $DATA_DIRS{test}/grammar.glue",
    $TEST_GRAMMAR,
  ...
  $cachepipe->cmd("test-extract-onebest-$run",
                  "java -Xmx20g -cp $JOSHUA/class -Dfile.encoding=utf8 joshua.util.ExtractTopCand $testrun/test.output.nbest.noOOV $output",
                  "$testrun/test.output.nbest.noOOV", 
                  $output);
...
if ($TEST_GRAMMAR_FILE) {
  ...
  if ($DO_FILTER_TM and ! $DOING_LATTICES) {
		...
		$cachepipe->cmd("filter-test-$NAME",
										"$CAT $GRAMMAR_FILE | java -Xmx20g -Dfile.encoding=utf8 -cp $JOSHUA/class joshua.tools.TestSetFilter $FILTERING -v $TEST{source} | $SCRIPTDIR/training/filter-rules.pl -bus$SCOPE | gzip -9n > $TEST_GRAMMAR",
										$GRAMMAR_FILE,
										$TEST{source},
										$TEST_GRAMMAR);
  }
...
if ($DO_MBR) {
  $cachepipe->cmd("test-$NAME-onebest-parmbr", 
									"cat $testrun/test.output.nbest.noOOV | java -Xmx20g -cp $JOSHUA/class -Dfile.encoding=utf8 joshua.decoder.NbestMinRiskReranker false 1 > $testrun/test.output.1best",
									"$testrun/test.output.nbest.noOOV", 
									"$testrun/test.output.1best");
} else {
  $cachepipe->cmd("test-$NAME-extract-onebest",
									"java -Xmx20g -cp $JOSHUA/class -Dfile.encoding=utf8 joshua.util.ExtractTopCand $testrun/test.output.nbest.noOOV $testrun/test.output.1best",
									"$testrun/test.output.nbest.noOOV", 
									"$testrun/test.output.1best");
}
